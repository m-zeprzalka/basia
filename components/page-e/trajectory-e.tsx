...
import { trajectory } from "@/data/page-e/copy"
...
/* Wykres liczony w pikselach: szerokość z pomiaru ramy, więc stopień pisma
   etykiet jest stały na każdej szerokości ekranu. */
const DOMAIN = { fast: 131.9, slow: 137.6 }
const PAD = { top: 44, right: 24, bottom: 34, left: 24 }
...
/**
 * 02 — Trajektoria. Wykres na 8 kolumnach, lista startów (tablica) na 4;
 * punkt i wiersz są jednym stanem — dotknięcie jednego podświetla drugie.
 */
export function TrajectoryE() {
  ...
  return (
    <Chapter id="trajektoria" index={2} tone="light">
        ...
        <Grid className="mt-14 items-start gap-y-10 sm:mt-20 lg:mt-24">
            ...
            <figure className="rounded-2xl border border-border p-4 sm:p-6">
              <figcaption className="flex flex-wrap items-baseline justify-between gap-x-6 gap-y-1">
                <span className="text-base font-medium sm:text-lg">
                  Droga od <span className="board-e">2:17,03</span> do{" "}
                  <span className="board-e text-gold-ink">2:12,45</span>
                </span>
                ...
                <div ref={chartRef}>
                  {width > 0 ? (
                    ...
                    >
                      ...
                      >
                        <tspan className="board-e">{seniorRecord.time}</tspan>
                          ...
                          <g key={point.time}>
                            ...
                            <text
                              ...
                              className={cn(
                                "board-e",
                                isLast
                                  ? "fill-gold-ink text-[15px] font-semibold"
              ...
              <p className="mt-4 text-xs leading-relaxed text-muted-foreground">
                {trajectory.caption}{" "}
                <span className="board-e">{seniorRecord.time}</span> (
          ...
          >
                  ...
                  <li key={point.time}>
                    ...
                    >
                      <span className="flex justify-center">
                        {point.medal ? (
                          <Medal medal={point.medal} className="size-8" />
                        ) : (
                          <span className="board-e text-xs text-muted-foreground">
                            {point.note?.match(/^\d+\./)?.[0] ?? "—"}
                          </span>
                        )}
                      </span>
            ...
            <p className="mt-4 text-sm leading-relaxed text-muted-foreground">
              Poprawa o blisko pięć sekund w dwanaście miesięcy — i najlepszy
              czas półfinałów zamieniony na złoto w finale.
            </p>
          </Reveal>
        </Grid>
      </Frame>
