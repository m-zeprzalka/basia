...
/**
 * 06 — Partnerstwo na papierze. Jedyna jasna plansza prezentacji — jak
 * kartka umowy między dwiema ciemnymi. Cztery argumenty ogromnymi numerami,
 * formaty, horyzont-tor i jeden przycisk.
 */
export function PartnersF() {
  return (
    <SectionF id="partnerstwo" index={6} tone="paper">
      <Frame className="py-24 sm:py-32 lg:py-40">
        ...
        <ol className="mt-16 grid gap-px overflow-hidden rounded-3xl bg-[rgba(11,15,22,0.12)] sm:mt-20 sm:grid-cols-2 lg:mt-24">
          {partners.arguments.map((argument, index) => (
            <Reveal
              as="li"
              key={argument.title}
              delay={index * 0.05}
              className="bg-paper-f p-6 sm:p-8 lg:p-10"
            >
              <p className="condensed-f text-[clamp(4rem,9vw,8rem)] text-[rgba(11,15,22,0.12)]">
        ...
        <Grid className="mt-16 gap-y-8 sm:mt-20 lg:mt-24">
          ...
          >
            <ol className="relative grid gap-6 sm:grid-cols-4">
              <span
                className="absolute top-[7px] right-0 left-0 hidden h-px bg-[repeating-linear-gradient(to_right,rgba(11,15,22,0.35)_0_8px,transparent_8px_16px)] sm:block"
                aria-hidden="true"
              />
              {partners.horizon.map((entry) => (
                <li key={entry.year} className="relative flex gap-4 sm:block">
                  <span
                    className={cn(
                      "relative z-10 mt-0.5 grid size-4 shrink-0 place-items-center rounded-full ring-4 ring-paper-f sm:mt-0",
                  ...
                  >
                    {entry.state === "done" ? (
                      <CheckIcon
                        className="size-2.5 text-paper-f"
                        strokeWidth={3}
                      />
                    ) : null}
                    {entry.state === "goal" ? (
