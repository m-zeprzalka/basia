...
import { introF } from "@/data/page-f/copy"

const FINAL_SECONDS = 132.45
const STORAGE_KEY = "bl-f-intro"
      ...
      <AnimatePresence>
        {phase === "count" || phase === "exit" ? (
          <motion.div
            ...
            className="fixed inset-0 z-[90] flex flex-col items-center justify-center bg-ink-f text-chalk-f"
          >
            <p className="tag-f text-chalk-f/60">{introF.label}</p>
            <p className="board-f mt-6 text-[clamp(3rem,16vw,12rem)] leading-none font-medium">
              <span ref={digitsRef}>0:00,00</span>
            </p>
            <div className="mt-8 h-px w-40 overflow-hidden bg-white/12">
              <motion.span
                className="block h-full origin-left bg-aqua-f"
                initial={{ scaleX: 0 }}
                animate={{ scaleX: 1 }}
            ...
            <button
              type="button"
              onClick={skip}
              className="tag-f absolute right-6 bottom-6 text-chalk-f/60 transition-colors hover:text-chalk-f"
            >
              {introF.skip}
            </button>
          </motion.div>
