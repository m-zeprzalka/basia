...
import { mediaF } from "@/data/page-f/copy"
...
export function MediaF() {
  ...
  return (
    <SectionF id="media" index={5} className="overflow-hidden">
...
function PhotoCard({ item, onOpen }: { item: PhotoItem; onOpen: () => void }) {
  ...
  return (
    <button
      ...
      className={cn(
        "group relative h-[52vh] shrink-0 snap-start overflow-hidden rounded-2xl bg-ink-f-2 text-left ring-1 ring-white/10 outline-none focus-visible:ring-3 focus-visible:ring-aqua-f/70 sm:h-[58vh] lg:h-[64vh]",
    ...
    >
      ...
      <span
        ...
        aria-hidden="true"
      />
      <span className="tag-f absolute top-4 left-4 rounded-full bg-ink-f/70 px-2.5 py-1 text-[0.5625rem] text-chalk-f backdrop-blur">
        {item.tag}
      </span>
      <span className="absolute inset-x-4 bottom-4 translate-y-2 text-sm font-medium text-chalk-f opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
        {item.caption}
      </span>
    </button>
  )
...
function VideoCard({
  ...
  return (
    <div className="group relative aspect-video h-[52vh] shrink-0 snap-start overflow-hidden rounded-2xl bg-ink-f-2 ring-1 ring-white/10 sm:h-[58vh] lg:h-[64vh]">
      {playing ? (
        <iframe
          src={src}
          title={item.title}
        ...
        <button
          ...
          onClick={onPlay}
          data-cursor="Odtwórz"
          className="absolute inset-0 size-full text-left outline-none focus-visible:ring-3 focus-visible:ring-aqua-f/70 focus-visible:ring-inset"
        ...
        >
          ...
          <span className="absolute top-4 left-4 flex items-center gap-2">
            <span className="tag-f rounded-full bg-chalk-f px-2.5 py-1 text-[0.5625rem] text-ink-f">
              Wideo
            </span>
            <span className="tag-f text-[0.5625rem] text-chalk-f/80">
              {item.tag}
            </span>
          </span>
          <span className="absolute inset-0 grid place-items-center">
            <span className="grid size-16 place-items-center rounded-full bg-chalk-f text-ink-f shadow-[0_12px_32px_-8px_rgba(0,0,0,0.6)] transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:scale-105 sm:size-20">
              <PlayIcon
          ...
          <span className="absolute inset-x-4 bottom-4">
            <span className="line-clamp-2 block font-heading text-sm leading-snug font-semibold tracking-tight text-chalk-f sm:text-base">
              {item.title}
            </span>
            <span className="mt-1 block text-[0.6875rem] text-chalk-f/65 sm:text-xs">
