...
import { parisF } from "@/data/page-f/copy"
    ...
    <SectionF id="paryz" index={4}>
      ...
      <div ref={stageRef} className="relative h-[150vh]">
        <div className="sticky top-0 h-svh overflow-hidden">
          ...
          <div className="absolute inset-0 bg-[linear-gradient(to_top,var(--ink-f)_0%,color-mix(in_oklab,var(--ink-f)_70%,transparent)_35%,color-mix(in_oklab,var(--ink-f)_15%,transparent)_70%,color-mix(in_oklab,var(--ink-f)_50%,transparent)_100%)]" />
            ...
            >
              <Grid className="items-end gap-y-6">
                <p className="tag-f col-span-4 flex items-center gap-4 sm:col-span-8 lg:col-span-12">
                  <span className="text-gold-f">{parisF.index}</span>
                  <span
                    className="h-px w-10 bg-current opacity-30"
                    aria-hidden="true"
                  />
                  <span className="text-chalk-f/70">{parisF.label}</span>
                </p>
                <h2 className="wide-f col-span-4 text-[clamp(2.25rem,8vw,6.5rem)] text-chalk-f sm:col-span-8 lg:col-span-9">
                  {parisF.title}
                </h2>
                <p className="col-span-4 max-w-[52ch] text-base leading-relaxed text-chalk-f/80 sm:col-span-6 sm:text-lg lg:col-span-4 lg:col-start-9 lg:pb-2">
                  {parisF.lead}
                </p>
              </Grid>
            </motion.div>
      ...
      <Frame className="pt-16 pb-24 sm:pb-32 lg:pb-40">
        {/* Trzy liczby. */}
        <Reveal>
          <ul className="grid grid-cols-3 gap-x-4 border-b border-white/10 pb-10 sm:gap-x-8">
            {parisData.takeaways.map((fact) => (
              <li key={fact.label} className="min-w-0">
                <p className="condensed-f text-[clamp(2rem,7vw,7rem)] text-chalk-f">
                  {fact.value}
                </p>
                <p className="mt-2 max-w-[22ch] text-xs leading-snug text-chalk-f/65 sm:text-sm">
                  {fact.label}
                </p>
              </li>
            ))}
        ...
        <Grid className="mt-14 items-start gap-y-12 sm:mt-20">
          <div className="col-span-4 sm:col-span-8 lg:col-span-7">
            <Reveal>
              <p className="tag-f text-chalk-f/55">Sztafety</p>
                  ...
                  >
                    <PlaceMark
                      ...
                      className={cn(
                        "size-11",
                        relay.highlight
                          ? "text-chalk-f ring-white/60"
                          : "text-chalk-f/60"
                      )}
                    />
                    <div className="min-w-0">
                      <p className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                        <span className="text-base leading-snug font-semibold text-chalk-f sm:text-lg">
                          {relay.event}
                        </span>
                        <span
                          className={cn(
                            ...
                            relay.stage === "finał"
                              ? "bg-gold-f text-ink-f"
                              : "bg-white/10 text-chalk-f/80"
                          )}
                        >
                          {relay.stage}
                        </span>
                      </p>
                      <p className="mt-1.5 text-sm text-chalk-f/60">
                        {relay.lineup}
                      </p>
                      {relay.note ? (
                        <p className="mt-1.5 text-sm text-chalk-f/85">
                          {relay.note}
                        </p>
                      ) : null}
                    </div>
                    <div className="col-start-2 mt-3 flex flex-wrap items-baseline gap-x-4 gap-y-1 sm:col-start-3 sm:mt-0 sm:flex-col sm:items-end sm:gap-1">
                      <p className="board-f text-2xl text-chalk-f sm:text-3xl">
                        {relay.time}
                      </p>
                      {relay.split ? (
                        <p className="text-xs text-chalk-f/60">
                          zmiana Barbary{" "}
            ...
            <Reveal delay={0.06} className="mt-10">
              <p className="tag-f text-chalk-f/55">Starty indywidualne</p>
              <ul className="mt-4 grid divide-y divide-white/10 border-y border-white/10 sm:grid-cols-2 sm:divide-x sm:divide-y-0">
                {parisData.individual.map((start) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="mt-0.5 text-xs text-chalk-f/55">
              ...
              <p className="mt-4 max-w-[60ch] text-sm leading-relaxed text-chalk-f/60">
          ...
          <div className="col-span-4 grid grid-cols-3 gap-3 sm:col-span-8 lg:col-span-4 lg:col-start-9 lg:grid-cols-1 lg:gap-4">
            {strip.map((item, index) => (
              <Reveal key={item.caption} delay={index * 0.06}>
                <figure className="relative aspect-[4/5] overflow-hidden rounded-xl ring-1 ring-white/10 lg:aspect-[16/10]">
                  <Image
                    src={item.src}
                    alt={item.alt}
                    fill
                  ...
                  />
                  <figcaption className="absolute inset-x-2 bottom-2 hidden text-[0.6875rem] text-chalk-f/85 sm:block lg:inset-x-3 lg:bottom-3 lg:text-xs">
                    <span className="rounded bg-ink-f/70 px-1.5 py-0.5 backdrop-blur">
                      {item.caption}
                    </span>
                  </figcaption>
                </figure>
        ...
        <Grid className="mt-16 sm:mt-20 lg:mt-24">
          <Reveal className="col-span-4 sm:col-span-8 lg:col-span-9 lg:col-start-2">
            <p className="tag-f text-chalk-f/55">{parisF.videoHeading}</p>
