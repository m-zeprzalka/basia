...
import { medalsG } from "@/data/page-g/copy"
...
export function MedalsG() {
  ...
  return (
    <SectionG id="medale" index={3} className="lanes-g">
        ...
        <Reveal delay={0.08}>
          <div className="relative mt-14 sm:mt-20 lg:mt-24">
            <ol
              ...
              className="relative divide-y divide-ink-g/10 border-y border-ink-g/10"
            >
              {medalCards.map((card, index) => (
                <li key={card.key}>
                  <button
                    ...
                    className={cn(
                      "group grid w-full grid-cols-[3rem_1fr] items-center gap-x-4 py-6 text-left transition-colors outline-none focus-visible:text-red-g sm:grid-cols-[4rem_1fr_auto] sm:gap-x-8 sm:py-8",
                      hovered && hovered !== card.key && "opacity-45"
                    )}
                    aria-label={`${card.title}, ${card.sub} — zobacz wyniki`}
                  >
                    <span className="tag-f text-gold-g">
                      {String(index + 1).padStart(2, "0")}
                    </span>
                    <span className="min-w-0">
                      <span className="flex flex-wrap items-center gap-x-4 gap-y-2">
                        <span className="condensed-f text-[clamp(2rem,6vw,4.5rem)] transition-colors group-hover:text-red-g">
                          {card.title}
                        </span>
                        {/* Miniatura tylko na dotyku (poniżej lg). */}
                        <span className="relative hidden aspect-[4/3] w-24 overflow-hidden rounded-lg ring-1 ring-ink-g/10 max-lg:inline-block sm:w-32">
                          <Thumb card={card} />
                        </span>
                      </span>
                      <span className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-2 text-sm text-ink-g/60">
                        <span>{card.sub}</span>
                        ...
                        {card.videos.length > 0 ? (
                          <span className="tag-f text-[0.5625rem] text-red-g">
                            Wideo
                          </span>
                        ) : null}
                      </span>
                    </span>
                    <span className="hidden items-center gap-2 text-sm text-ink-g/60 transition-colors group-hover:text-ink-g sm:flex">
                      {card.medals.length}{" "}
              ...
              <li>
                <button
                  ...
                  className="group grid w-full grid-cols-[3rem_1fr] items-center gap-x-4 py-6 text-left outline-none focus-visible:text-red-g sm:grid-cols-[4rem_1fr_auto] sm:gap-x-8 sm:py-8"
                >
                  <span className="tag-f text-gold-g">05</span>
                  <span className="min-w-0">
                    <span className="condensed-f flex flex-wrap items-baseline gap-x-4 text-[clamp(2rem,6vw,4.5rem)] transition-colors group-hover:text-red-g">
                      {medalsG.recordsRow.title}
                      <span className="text-gold-g">14</span>
                    </span>
                    <span className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-sm text-ink-g/60">
                ...
                <AnimatePresence>
                  {hoveredCard ? (
                    <motion.div
                      ...
                      className="relative aspect-[4/5] w-56 -translate-x-1/2 -translate-y-[60%] overflow-hidden rounded-2xl shadow-[0_30px_60px_-20px_rgba(11,15,22,0.35)] ring-1 ring-ink-g/15 xl:w-64"
                    >
