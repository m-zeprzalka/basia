...
import { medalWordFor, type MedalsDict, type VideoUiDict } from "@/data/main/types"
...
import { Medal, PlaceMark } from "@/components/main/medal"
import { RecordsCompact } from "@/components/main/records"
 ...
 * okno z pełnymi wynikami i — tam, gdzie są — nagraniami PZP; drugi przycisk
 * otwiera pełne tabele rekordów z medalami przy wyścigach rekordowych.
 */
export function Medals({
  ...
  const openCard = t.cards.find((card) => card.key === openKey) ?? null

  return (
    <Chapter id={id} index={3} tone="light">
      ...
      >
        ...
        >
          {openCard ? (
            <MilestoneSheet card={openCard} t={t} videoUi={videoUi} />
          ) : (
            <DialogTitle className="sr-only">{t.resultsFallback}</DialogTitle>
          )}
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={recordsOpen} onOpenChange={setRecordsOpen}>
        ...
        >
          <div className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b border-border bg-background/95 px-5 py-4 backdrop-blur sm:px-8">
            <div>
              <DialogTitle className="font-heading text-lg font-semibold tracking-tight sm:text-xl">
                {t.recordsDialog.title}
              </DialogTitle>
              <DialogDescription className="mt-1">
                {t.recordsDialog.desc}
              </DialogDescription>
            </div>
            <DialogClose
          ...
          <div className="px-5 py-6 sm:px-8 sm:py-8">
            <RecordsCompact t={t.records} medalLabels={t.medalLabels} />
            <div className="mt-10 border-t border-border pt-8">
              <h4 className="eyebrow text-muted-foreground">
                {t.recordsDialog.milestonesHeading}
              </h4>
              <ul className="mt-4 grid gap-6 sm:grid-cols-3">
                {t.recordsDialog.recordMilestones.map((milestone) => (
                  <li
                    key={milestone.id}
...
function MedalCardTile({
  ...
  return (
    <button
      ...
      aria-label={`${card.title}, ${card.sub} — ${t.cardSrOpen}`}
    >
      ...
      {card.videos.length > 0 ? (
        <span className="absolute top-3 right-3 grid size-8 place-items-center rounded-full bg-white/90 text-navy sm:top-4 sm:right-4 sm:size-9">
          <PlayIcon
            ...
            aria-hidden="true"
          />
          <span className="sr-only">{t.videoBadgeSr}</span>
        </span>
      ) : null}
      ...
      <span className="absolute inset-x-3 bottom-3 sm:inset-x-4 sm:bottom-4">
        ...
        <span className="mt-3 inline-flex items-center gap-1.5 text-xs font-medium text-white/85">
          {card.medals.length} {medalWordFor(card.medals.length, t.medalWord)}
          <ArrowUpRightIcon
          ...
          <div className="mt-8">
            ...
            >
              {card.videos.map((video) => (
                <Video key={video.id} video={video} t={videoUi} compact />
              ))}
            </div>
          </div>
        ) : null}

        <p className="mt-6 text-xs text-muted-foreground">{t.dialogHint}.</p>
      </div>
    </div>
  )
}
