...
import type { MediaDict, VideoUiDict } from "@/data/main/types"
import type {
  MediaItem,
  PhotoItem,
  VideoItem,
...
import { VideoFrame } from "@/components/main/video"

type FilterKey = MediaDict["filters"][number]["key"]
 ...
 * W ścianie są też dwa publiczne reelsy z Facebooka, a pod ścianą —
 * artykuł Przeglądu Sportowego i profil Barbary na Instagramie.
 */
export function Media({
  ...
  const filtered = React.useMemo(
    () => t.items.filter((item) => matches(item, filter)),
    [t.items, filter]
  )
  ...
  return (
    <Chapter id={id} index={5} tone="light">
            ...
            aside={
              ...
              >
                {t.filters.map((entry) => {
                  ...
                  const count = t.items.filter((item) =>
          ...
          >
            ...
            >
              ...
              <div className="relative">
                <p className="eyebrow text-white/70">{t.instagram.eyebrow}</p>
      ...
      >
        ...
        >
          <DialogTitle className="sr-only">
            {active?.caption ?? t.lightbox.fallback}
          </DialogTitle>
          {active ? (
...
function VideoTile({
  ...
  return (
    <div className="group absolute inset-0 overflow-hidden rounded-2xl bg-navy-deep ring-1 ring-foreground/10">
      {playing ? (
        <VideoFrame video={item} lang={videoUi.embedLang} />
      ) : (
        <button
          type="button"
          onClick={onPlay}
          ...
          aria-label={`${videoUi.playPrefix} ${item.title}`}
        >
          <Image
            src={item.poster}
            alt=""
