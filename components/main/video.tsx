...
import type { VideoUiDict } from "@/data/main/types"
 ...
 *
 * Etykiety („Wideo", „Odtwórz nagranie:") i język interfejsu YouTube
 * przychodzą ze słownika bieżącej wersji językowej.
 */

export function videoEmbedSrc(video: VideoItem, lang: string) {
  if (video.provider === "youtube") {
    return `https://www.youtube-nocookie.com/embed/${video.youtubeId}?autoplay=1&rel=0&modestbranding=1&playsinline=1&hl=${lang}`
  }
  ...
  return (
    ...
    >
      <div className="relative aspect-video w-full">
        {playing ? (
          <VideoFrame video={video} lang={t.embedLang} />
        ) : (
          <button
            type="button"
            ...
            aria-label={`${t.playPrefix} ${video.title}`}
          >
            <Image
              src={video.poster}
              alt=""
