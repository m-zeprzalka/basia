...
import { replay } from "@/data/page-h/copy"
 ...
 * prędkością, i zamierają w chwili, gdy rekord dotyka ściany. Fotofinisz
 * pokazuje ostatnie 1,1 % dystansu w powiększeniu: tam 0,32 s ma szerokość
 * dłoni. Wersja H rysuje się tokenami głębokiej wody wariantu E; etykiety
 * mają stałe odstępy w rem, więc nie nachodzą na kropki ani na siebie
 * na wąskich ekranach.
 */
export function ReplayH() {
  ...
  const record = lanes.find((lane) => "record" in lane && lane.record)!
  ...
  return (
    ...
    >
      ...
      {/* Fotofinisz — powiększenie ostatnich 1,1 % dystansu. Etykiety rekordu
          leżą nad linią, etykiety Barbary pod nią — dwa piętra zamiast dwóch
          kolizyjnych wierszy, więc napisy nie nachodzą na siebie na mobile. */}
      <motion.div
      ...
      >
        <div className="rounded-2xl border border-gold-bright/30 bg-navy-deep/70 p-4 sm:p-6">
          <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1">
            <p className="eyebrow text-gold-bright">{replay.finish.heading}</p>
            <p className="eyebrow text-white/45">{replay.finish.scale}</p>
          </div>
          <div className="relative mt-6 h-20">
            ...
            />
            <span
              className="absolute top-3 right-0 bottom-3 w-px bg-gold-bright"
              aria-hidden="true"
            />
            {/* Odcinek 0,32 s między zawodniczką a ścianą. */}
            <span
              className="absolute top-1/2 h-0.5 -translate-y-1/2 bg-[linear-gradient(to_right,var(--aqua),var(--gold-bright))]"
              style={{ left: `${barbaraZoom}%`, right: 0 }}
              aria-hidden="true"
            />
            <span
              className="absolute top-1/2 size-4 -translate-x-1/2 -translate-y-1/2 rounded-full bg-aqua"
              style={{ left: `${barbaraZoom}%` }}
              aria-hidden="true"
            />
            <span
              className="absolute top-1/2 right-0 size-4 translate-x-1/2 -translate-y-1/2 rounded-full bg-gold-bright"
              aria-hidden="true"
            />
            {/* Nad linią: rekord przy ścianie. */}
            <p className="absolute top-0 right-0 text-right whitespace-nowrap">
              <span className="board-e text-xs text-gold-bright">
                {record.time}
              </span>
              <span className="text-[0.6875rem] text-white/60">
                {" "}
                · {replay.finish.recordLabel}
              </span>
            </p>
            {/* Pod linią: Barbara, dosunięta do własnej kropki od lewej. */}
            <p
              className="absolute bottom-0 -translate-x-full text-right whitespace-nowrap"
              style={{ left: `calc(${barbaraZoom}% - 0.625rem)` }}
            >
              <span className="board-e text-xs text-aqua">{barbara.time}</span>
              <span className="text-[0.6875rem] text-white/60">
                {" "}
                · {replay.finish.barbaraLabel}
              </span>
            </p>
            {/* Różnica — pod odcinkiem. */}
            <span
              className="board-e absolute bottom-0 -translate-x-1/2 text-xs whitespace-nowrap text-gold-bright"
              style={{ left: `${(barbaraZoom + 100) / 2}%` }}
            >
              0,32 s
            </span>
          </div>
        ...
        {/* Dwie liczby, które zostają w głowie. */}
