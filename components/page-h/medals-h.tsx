...
import { RecordsCompactH } from "@/components/page-h/records-h"
 ...
 *
 * Wersja H: kadry z nagrań to miniatury 16:9 w pełnej rozdzielczości
 * (maxresdefault) — kryją całą kartę bez czarnych pasów; okno tabel
 * rekordów pokazuje medale zdobyte w wyścigach rekordowych.
 */
export function MedalsH() {
          ...
          <div className="mt-8">
            ...
            >
              {card.videos.map((video) => (
                <VideoH key={video.id} video={video} compact />
              ))}
            </div>
          </div>
        ) : null}
